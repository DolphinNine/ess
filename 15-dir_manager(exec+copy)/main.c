...
/*Данные для этапа копирования------------------------------------------------*/
  char cp_file_name[255] = {0}; /*Хранение имени файла*/
  char cp_file_cwd[PATH_MAX] = {0}; /*Хранение пути файла*/
  struct stat file_info; /*Хранение данных о файле*/
/*----------------------------------------------------------------------------*/
  ...
  init_pair(4, COLOR_WHITE, COLOR_GREEN); /*Пара для сообщений*/

  attron(COLOR_PAIR(2));
  bkgd(COLOR_PAIR(2));
  mvprintw(0, 2, "Enter - Open | Tab - Switch window | F3 - Copy | F4 - Paste | F12 - Exit");
...
/*---------------------------------------------------------------------------*/
/*F3*/
      /*Копировать файл. На самом деле, по нажатию клавиши лишь проверяется
        файл ли это, а не директория. Если всё в порядке - имя файла
        запоминается для последующего использования*/
      case (char)KEY_F(3):
        getyx(*active_wnd, y, x); /*Считать позицию нажатия*/
        /*Позиция используется для получение выделенного файла из структуры*/
        stat((*dir_entry_ptr)[y + *relative_y_ptr]->d_name, &file_info);
        /*Если это не директория - запомнить имя и путь*/
        /*(будет запоминать вообще всё, что не директория)*/
        if (S_ISDIR(file_info.st_mode) == 0)
        {
          strcpy(cp_file_name, (*dir_entry_ptr)[y + *relative_y_ptr]->d_name);
          realpath(cp_file_name, cp_file_cwd);
          print_message(*active_wnd, 1, "COPY: Can try to copy");
          break;
        }
        print_message(*active_wnd, -1, "COPY: Can't copy directories yet");
      break;

/*---------------------------------------------------------------------------*/
/*F4*/
      /*Вставить файл. Если файл не был "скопирован" ранее, кейзом выше, то
        то здесь ничего сделано не будет*/
      case (char)KEY_F(4):
        /*Проверка - было ли что-то записано/удачно записано на шаге
          "копирования"(F3)*/
        if (strlen(cp_file_name) != 0)
        {
          /*Что-то было скопировано - можно начинать вставку. Вызов рутины.*/
          if(cp_rountine(*active_wnd, cp_file_name, cp_file_cwd) == 0)
          {
            /*Когда рутина окончила свою работу - обнулить данные о файле, чтобы
            в следующий раз вставлять было нечего, пока не будет повторено
            "копирование"*/
            memset(cp_file_name, 0, sizeof(cp_file_name));
            memset(cp_file_cwd, 0, sizeof(cp_file_cwd));
            /*Обновить указатель и вывести директорию по новой. В ней, теперь,
            должен будет появиться новый файл.*/
            *dir_entry_ptr = get_req(*active_wnd, *dir_entry_ptr, entries_amount_ptr, relative_y_ptr, (*dir_entry_ptr)[0]->d_name);
            print_message(*active_wnd, 1, "PASTE: Copied successfully");
          }
          else
          {
            print_message(*active_wnd, -1, "PASTE: Something gone wrong");
            memset(cp_file_name, 0, sizeof(cp_file_name));
            memset(cp_file_cwd, 0, sizeof(cp_file_cwd));
          }
        }
        ...
        {
          print_message(*active_wnd, -1, "PASTE: Nothing to paste!");
        }
      break;
