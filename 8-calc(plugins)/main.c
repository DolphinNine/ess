...
int main()
{
	...
	while(1)
	{
		...
		for (int i = 0; i < ls_size-1; i++) //ls_size всё ещё больше на единицу, чем нужно, потому уменьшается
		{
			printf("(%d) - %s\n", i+1, libs[i].name);
		}
		printf("(0) - Quit\n"\
		...
		if(choice > ls_size-1) //Число выбора выходит за пределы доступных опций
		{
